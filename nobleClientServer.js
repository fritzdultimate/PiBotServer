...
const settings = await ColemanSettings.findOne({ name: 'noble' });
...
setInterval(() => autoPrepareForClaiming('noble', MAIN_ADDRESS, activeSponsors), 1000);
setInterval(() => autoSubmitXDR('noble'), 100);
...
setInterval(async() => {
    ...
    if (upcomingClaimables.length > 0) {
        for(const claimable of upcomingClaimables) {
            if(claimable.name !== 'noble') continue;
