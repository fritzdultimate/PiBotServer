...
const settings = await ColemanSettings.findOne({ name: 'bot1' });
...
const sponsors = await Sponsors.find({ name: 'bot1' });
// const chunkSize = Math.floor(sponsors.length/2);


setInterval(() => autoPrepareForClaiming('bot1', MAIN_ADDRESS, activeSponsors), 1000);
setInterval(() => autoSubmitXDR('bot1'), 100);
        ...
        for (const p of usingSponsors) {
            try {
                ...
                if((change < 0) && (botBalance > Math.abs(change))) {
                    ...
                    if (success.hash) {
                        console.log(`✅ bot1 funded ${result.amount} Pi. Hash: ${success.hash}`);
                        
                    } else {
                        // console.log(`❌ bot1 Failed to fund ${result.amount} PI}`);
                    }
                } else {
...
setInterval(async() => {
    ...
    if (upcomingClaimables.length > 0) {
        for(const claimable of upcomingClaimables) {
            if(claimable.name !== 'bot1') continue;
            await sweepWallet(claimable.mnemonic, MAIN_ADDRESS);
            await sleep(1000);
        }
    } else {
        const readyPassphrases = await Passphrase.find({ name: 'bot1' });
